...
$APPLICATION->SetTitle("Проекты ТЕСТ");
global $USER;
$UserID=$USER->GetID();
$usersID=[];
?>
...
foreach($projects as $project){
	...
	$projectHours=round($projectMinutes/60, 0, PHP_ROUND_HALF_DOWN);//переводим в часы
...
$allclosedhours=$allhours-$allclosedhours;
	?>
	<div class="upper" onclick="showTasks('img-p-active','p-active')">
		<div class="allhours"><?=$allhours." / ".$allfacthours." / ".$allclosedhours?></div>
...
foreach($projects as $project){
	...
	$projectHours=round($projectMinutes/60);//переводим в часы
	$projectMinutes=$projectMinutes%60;
...
<?//ЗАДАЧИ БЕЗ ПРОЕКТА
	...
	while($t = $res->GetNext()){
		...
		$hours=round($minutes/60, 0, PHP_ROUND_HALF_DOWN);//переводим в часы
		$minutes=$minutes%60;
